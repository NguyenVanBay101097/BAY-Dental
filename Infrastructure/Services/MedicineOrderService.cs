...
namespace Infrastructure.Services
{
    public class MedicineOrderService : BaseService<MedicineOrder>, IMedicineOrderService
    {
        ...
        public async Task<PagedResult2<MedicineOrderBasic>> GetPagedResultAsync(MedicineOrderPaged val)
        {
            ...
            query = query.Include(x => x.Partner).Include(x => x.Employee).Include(x => x.Journal).Include(x => x.ToaThuoc).ThenInclude(s => s.SaleOrder);
        ...
        public async Task GenerateLogMedicineOrder(MedicineOrder payment)
        {
            ...
            var bodyContent = string.Format("{0} {1} số tiền {2} đồng", content, payment.ToaThuoc.Name, string.Format("{#,##0}", payment.Amount));
