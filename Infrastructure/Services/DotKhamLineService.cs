...
namespace Infrastructure.Services
{
    public class DotKhamLineService : BaseService<DotKhamLine>, IDotKhamLineService
    {
        ...
        {
            var dkl = await SearchQuery(x => x.Id == id).Include(x => x.Operations).FirstOrDefaultAsync();
        ...
        {
            var dkl = await SearchQuery(x => x.Id == val.Id).Include(x => x.Operations).FirstOrDefaultAsync();
            if (dkl.State != "draft")
        ...
        {
            var self = await SearchQuery(x => ids.Contains(x.Id)).Include(x => x.Routing)
                .Include("Routing.Lines")
                .Include("Routing.Lines.Product").ToListAsync();
            foreach(var line in self)
            {
