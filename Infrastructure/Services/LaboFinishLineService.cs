...
namespace Infrastructure.Services
{
    public class LaboFinishLineService : BaseService<LaboFinishLine>, ILaboFinishLineService
    {
        ...
        public async Task<IEnumerable<LaboFinishLineSimple>> Autocomplete(LaboFinishLinePageSimple val)
        {
            var query = SearchQuery();
           if(!string.IsNullOrEmpty(val.Search))
            ...
            return await query.OrderBy(x => x.Name).Select(x => new LaboFinishLineSimple
            {
                Id = x.Id,
                Name = x.Name
            }).ToListAsync();
