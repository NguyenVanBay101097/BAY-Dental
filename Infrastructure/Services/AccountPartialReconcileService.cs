...
namespace Infrastructure.Services
{
    public class AccountPartialReconcileService : BaseService<AccountPartialReconcile>, IAccountPartialReconcileService
    {
        ...
        public override ISpecification<AccountPartialReconcile> RuleDomainGet(IRRule rule)
        {
            var companyId = CompanyId;
            switch (rule.Code)
            {
                case "account.partial.reconcicle_comp_rule":
                    return new InitialSpecification<AccountPartialReconcile>(x => x.CompanyId == companyId);
                default:
                    return null;
            }
        }
