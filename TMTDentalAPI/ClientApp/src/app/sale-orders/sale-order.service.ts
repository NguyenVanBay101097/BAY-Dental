...
} from "../account-payments/account-register-payment.service";
import {
  AccountPaymentPaged,
  AccountPaymentBasic,
} from "../account-payments/account-payment.service";
...
export class SaleOrderService {
  ...
  getPaged(val: any): Observable<PagedResult2<SaleOrderBasic>> {
    return this.http.get<PagedResult2<SaleOrderBasic>>(
      this.baseApi + this.apiUrl,
      { params: new HttpParams({ fromObject: val }) }
    );
  }

  get(id): Observable<SaleOrderDisplay> {
    return this.http.get<SaleOrderDisplay>(
  ...
  actionConvertToOrder(id: string) {
    return this.http.post(
      this.baseApi + this.apiUrl + "/" + id + "/ActionConvertToOrder",
      {}
    );
  }
  ...
  checkPromotion(id: string) {
    return this.http.get(
      this.baseApi + this.apiUrl + "/" + id + "/CheckPromotion"
    );
  }

  actionInvoiceCreateV2(id: string) {
    return this.http.post(
      this.baseApi + this.apiUrl + "/" + id + "/ActionInvoiceCreateV2",
      {}
    );
  }
  ...
  applyServiceCards(data: any) {
    return this.http.post(
      this.baseApi + this.apiUrl + "/ApplyServiceCards",
      data
    );
  }
  ...
  applyPromotion(id: string) {
    return this.http.post(
      this.baseApi + this.apiUrl + "/" + id + "/ApplyPromotion",
      {}
    );
  }
  ...
  getDotKhamList(id: string): Observable<DotKhamBasic[]> {
    return this.http.get<DotKhamBasic[]>(
      this.baseApi + this.apiUrl + `/${id}/GetDotKhamList`
    );
  }

  getDefaultRegisterPayment(id: string) {
    return this.http.get(
      this.baseApi + "api/Partners/" + id + "/GetDefaultRegisterPayment"
    );
  }

  defaultGetInvoice(ids: string[]) {
    return this.http.post(
      this.baseApi + this.apiUrl + "/DefaultGetInvoice",
      ids
    );
  }

  defaultValGet(
    val: AccountRegisterPaymentDefaultGet
  ): Observable<AccountRegisterPaymentDisplay> {
    return this.http.post<AccountRegisterPaymentDisplay>(
      this.baseApi + "api/accountregisterpayments/defaultget",
      val
    );
  }
  ...
  defaultOrderGet(
    ...
    return this.http.post<AccountRegisterPaymentDisplay>(
      this.baseApi + "api/accountregisterpayments/OrderDefaultGet",
      val
    );
  }

  getPaymentBasicList(val): Observable<AccountPaymentBasic[]> {
    return this.http.get<AccountPaymentBasic[]>(
      this.baseApi + "api/AccountPayments/GetPaymentBasicList",
      { params: val }
    );
  }

  getAccountPaymentReconcicles(id): Observable<PaymentInfoContent[]> {
    return this.http.get<PaymentInfoContent[]>(
      this.baseApi + `api/AccountInvoices/${id}/GetSaleOrderPaymentInfoJson`
    );
  }

  getPayments(id): Observable<PaymentInfoContent[]> {
    return this.http.get<PaymentInfoContent[]>(
      this.baseApi + this.apiUrl + `/${id}/GetPayments`
    );
  }
