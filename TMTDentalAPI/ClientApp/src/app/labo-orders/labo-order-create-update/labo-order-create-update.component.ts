...
export class LaboOrderCreateUpdateComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.route.queryParamMap
      ...
      .subscribe((result) => {
        ...
        if (result.saleOrder) {
          this.filteredSaleOrders = _.unionBy(
            this.filteredSaleOrders,
            [result.saleOrder],
            "id"
          );
        }
  ...
  printLaboOrder() {
    if (this.id) {
      this.laboOrderService.getPrint(this.id).subscribe((result: any) => {
        ...
        setTimeout(() => {
          var printContents = document.getElementById("printLaboOrderDiv")
            .innerHTML;
  ...
  onSaveConfirm() {
    ...
    val.dateOrder = this.intlService.formatDate(
      val.dateOrderObj,
      "yyyy-MM-ddTHH:mm:ss"
    );
    val.datePlanned = val.datePlannedObj
      ? this.intlService.formatDate(val.datePlannedObj, "yyyy-MM-ddTHH:mm:ss")
      : null;
  ...
  loadRecord() {
    if (this.id) {
      this.laboOrderService.get(this.id).subscribe((result) => {
        ...
        if (result.partner) {
          this.filteredPartners = _.unionBy(
