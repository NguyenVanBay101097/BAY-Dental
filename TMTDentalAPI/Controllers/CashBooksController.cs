...
namespace TMTDentalAPI.Controllers
{
    ...
    public class CashBooksController : ControllerBase
    {
        ...
        {
            var res = await _cashBookService.GetDetails(val.DateFrom, val.DateTo, val.Limit, val.Offset, val.CompanyId, val.Search, val.ResultSelection , val.JournalId, val.AccountCode, val.PaymentType);
            return Ok(res);
        }
        ...
        public async Task<IActionResult> ExportExcelFile(CashBookDetailFilter val)
        {
            ...
            val.Offset = 0;
            var services = await _cashBookService.GetDetails(val.DateFrom, val.DateTo, val.Limit, val.Offset, val.CompanyId.Value, val.Search, val.ResultSelection , val.JournalId, val.AccountCode, val.PaymentType);
