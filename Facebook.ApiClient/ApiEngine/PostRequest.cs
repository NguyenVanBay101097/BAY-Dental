...
namespace Facebook.ApiClient.ApiEngine
{
    ...
    public class PostRequest : ApiRequest, IPostRequest
    {
        ...
        internal PostRequest(string requestUrl, ApiClient client, bool legacy = false) : base(requestUrl, client,
            ApiRequestHttpMethod.POST, legacy)
        {
        }
        ...
        {
            StartTimer();
            var response = await RestClient.ExecuteAsync<TEntity>(RestRequest);
            StopTimer();
