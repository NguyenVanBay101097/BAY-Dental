...
namespace Facebook.ApiClient.ApiEngine
{
    ...
    public class PagedRequest  : ApiRequest, IPagedRequest
    {
        ...
        internal PagedRequest(string requestUrl, ApiClient client, bool legacy = false, int limit = 50) : base(requestUrl, client, ApiRequestHttpMethod.GET, legacy)
        {
            this.Limit = limit;
        }
        ...
        {
            StartTimer();
            var response = await RestClient.ExecuteAsync<PagedResponse<TEntity>>(RestRequest);
            StopTimer();
